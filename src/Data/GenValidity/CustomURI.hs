...
instance Validity URI where
  validate uri = check condition ("Could not parse " <> show uri)
    where
      condition = isJust $ parseURI $ show uri

-- | Generate an 'URI' that parses using 'parseURIReference'.
genURIReference :: Gen URI
genURIReference = oneof [genURI, genRelativeReference]
...
genURI = (`suchThatMap` (parseURI . dangerousURIToString)) $ do
  ...
  uriPath <- case uriAuthority of
    ...
    Nothing -> oneof [genPathAbsolute, genPathNoScheme, genPathEmpty]
  uriQuery <- genQuery
...
genFragment =
  nullOrPrepend '#'
    . concat
    <$> genListOf (frequency [(4, genPathChar), (1, elements ["/", "?"])])
...
genCharUnreserved =
  frequency
    [(1, elements ['+', '-', '.', '~']), (4, genCharALPHA), (3, genCharDIGIT)]

genCharReserved :: Gen Char
genCharReserved = oneof [genCharGenDelim, genCharSubDelim]

genCharGenDelim :: Gen Char
genCharGenDelim = elements [':', '/', '?', '#', '[', ']', '@']
...
genCharALPHA =
  Char.chr <$> oneof [choose (0x41, 0x5A), choose (0x61, 0x7A)]

genCharDIGIT :: Gen Char
genCharDIGIT = Char.chr <$> choose (0x30, 0x39)
